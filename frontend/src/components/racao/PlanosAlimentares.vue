  ...
  <div class="planos-alimentares-container">
        ...
        >
          <q-card-section class="text-center q-pa-sm">
            <div class="text-h4 text-blue">
              {{ racaoStore.formatarMoeda(estatisticas.custoMensalEstimado) }}
            </div>
    ...
    >
      ...
      <template v-slot:body-cell-peso_dieta="props">
        <q-td :props="props">
          <div class="text-weight-medium">
            {{ racaoStore.formatarPeso(props.row.PESO_REFERENCIA) }}
          </div>
      ...
      <template v-slot:body-cell-refeicoes_produtos="props">
        <q-td :props="props">
          <div class="text-weight-medium">
            {{ props.row.NUMERO_REFEICOES }}x refeições
          </div>
          <div class="text-caption">
            {{ props.row.total_produtos || 0 }} produtos
          </div>
        </q-td>
      </template>
      ...
      <template v-slot:body-cell-periodo="props">
        <q-td :props="props">
          ...
          <div
            v-if="props.row.DATA_FIM"
            class="text-caption"
          >
            até {{ formatarData(props.row.DATA_FIM) }}
          </div>
          <div
            v-else
    ...
    >
      <q-card style="min-width: 700px">
        ...
        <q-card-section>
          <q-form
            @submit="submitForm"
            class="q-gutter-md justify-between"
          >
            <!-- Animal e Categoria -->
            ...
            <div class="row q-gutter-md justify-between">
              <q-input
                ...
                :suffix="
                  'kg (' +
                  racaoStore.formatarPercentual(percentualPesoVivo) +
                  ' PV)'
                "
                class="col-4"
              />
              <q-input
                ...
                max="4"
                :rules="[
                  val => (val >= 1 && val <= 4) || 'Entre 1 e 4 refeições',
                ]"
                class="col-3"
              />
              <q-select
              ...
              <q-card-section>
                ...
                <div class="row q-gutter-md">
                  <div class="col">
                    ...
                    <div class="text-weight-medium">
                      {{
                        racaoStore.formatarPeso(
                          sugestaoNutricional.quantidade_sugerida_kg
                        )
                      }}/dia
                    </div>
                  </div>
                  <div class="col">
                    ...
                    <div class="text-weight-medium">
                      {{
                        racaoStore.formatarPercentual(
                          sugestaoNutricional.percentual_peso_vivo
                        )
                      }}
                    </div>
                  </div>
      ...
      <q-card style="min-width: 600px">
        <q-card-section>
          ...
          <div class="text-subtitle2 text-grey-6">
            {{ racaoStore.getCategoriaLabel(viewData?.CATEGORIA_NUTRICIONAL) }}
          </div>
        </q-card-section>
              ...
              <q-list>
                <q-item>
                  <q-item-section>
                    ...
                    <q-item-label>
                      {{ racaoStore.formatarPeso(viewData.PESO_REFERENCIA) }}
                    </q-item-label>
                  </q-item-section>
                  <q-item-section>
                    ...
                    <q-item-label>
                      {{
                        racaoStore.formatarPeso(
                          viewData.QUANTIDADE_DIARIA_TOTAL
                        )
                      }}
                    </q-item-label>
                  </q-item-section>
                ...
                <q-item>
                  ...
                  <q-item-section>
                    ...
                    <q-item-label>
                      {{
                        racaoStore.formatarMoeda(viewData.custo_diario_estimado)
                      }}
                    </q-item-label>
                  </q-item-section>
                </q-item>
                ...
                <q-item>
                  <q-item-section>
                    ...
                    <q-item-label>
                      ...
                      {{
                        viewData.DATA_FIM
                          ? ' até ' + formatarData(viewData.DATA_FIM)
                          : ' (em andamento)'
                      }}
                    </q-item-label>
                  </q-item-section>
        ...
        <q-card-section>
          ...
          >
            ...
            <template v-slot:body-cell-quantidades="props">
              <q-td :props="props">
                <div class="text-weight-medium">
                  {{ racaoStore.formatarPeso(props.row.QUANTIDADE_DIARIA) }}/dia
                </div>
                <div class="text-caption">
                  {{
                    racaoStore.formatarPeso(props.row.QUANTIDADE_POR_REFEICAO)
                  }}/refeição
                </div>
              </q-td>
            </template>
            ...
            <template v-slot:body-cell-horarios="props">
              <q-td :props="props">
                <div
                  v-if="props.row.HORARIO_REFEICAO_1"
                  class="text-caption"
                >
                  1ª: {{ props.row.HORARIO_REFEICAO_1 }}
                </div>
                <div
                  v-if="props.row.HORARIO_REFEICAO_2"
                  class="text-caption"
                >
                  2ª: {{ props.row.HORARIO_REFEICAO_2 }}
                </div>
                <div
                  v-if="props.row.HORARIO_REFEICAO_3"
                  class="text-caption"
                >
                  3ª: {{ props.row.HORARIO_REFEICAO_3 }}
                </div>
                <div
                  v-if="props.row.HORARIO_REFEICAO_4"
                  class="text-caption"
                >
                  4ª: {{ props.row.HORARIO_REFEICAO_4 }}
                </div>
              </q-td>
            </template>
    ...
    >
      <q-card>
        ...
        <q-card-section>
          Remover
          <strong>{{ itemParaExcluir?.produto_nome }}</strong>
          do plano?
        </q-card-section>
        <q-card-actions align="right">
          <q-btn
