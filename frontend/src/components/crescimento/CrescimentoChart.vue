<template>
  <div class="crescimento-chart">
    <div v-if="dadosProcessados.length === 0" class="text-center q-pa-xl">
      <q-icon name="show_chart" size="3rem" color="grey" />
    ...
    <div v-else class="chart-container" :style="{ height }">
      ...
      <!-- Canvas do gráfico -->
      <canvas ref="chartCanvas" :style="{ height: alturaCanvas }"></canvas>
      
      <!-- Legenda personalizada -->
      <div class="chart-legend q-mt-md">
        <div class="row q-gutter-md items-center justify-center">
          <div v-for="item in legendaItems" :key="item.label" class="row items-center">
            <div 
      ...
      <div v-if="estatisticas" class="chart-stats q-mt-md">
        <q-card flat bordered>
          <q-card-section class="q-pa-sm">
            <div class="row q-gutter-md text-center">
              ...
              <div class="col">
                <div class="text-h6" :class="estatisticas.tendencia > 0 ? 'text-positive' : 'text-negative'">
                  {{ estatisticas.tendencia > 0 ? '+' : '' }}{{ estatisticas.tendencia }}
                </div>
                <div class="text-caption">{{ tipo === 'peso' ? 'kg' : 'cm' }} (tendência)</div>
              </div>
              <div class="col">
                <div class="text-h6 text-accent">{{ estatisticas.periodo }}d</div>
                <div class="text-caption">Período</div>
              </div>
              <div class="col">
                <div class="text-h6 text-secondary">{{ estatisticas.media }}</div>
                <div class="text-caption">{{ tipo === 'peso' ? 'kg' : 'cm' }} (média)</div>
              </div>
            </div>
          </q-card-section>
        </q-card>
...
const estatisticas = computed(() => {
  ...
  const dataFinal = new Date(dadosProcessados.value[dadosProcessados.value.length - 1].data)
...
function configurarGrafico() {
  ...
  const datasets = [{
    ...
    backgroundColor: props.tipo === 'peso' ? 'rgba(25, 118, 210, 0.1)' : 'rgba(56, 142, 60, 0.1)',
    borderWidth: 2,
    pointRadius: 4,
    pointHoverRadius: 6,
  ...
  if (props.showTrend && valores.length > 1) {
    const dadosTendencia = valores.map((valor, index) => ({ x: index, y: valor }))
