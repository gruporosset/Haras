      ...
      <q-card class="col-3">
        <q-card-section class="text-center">
          <div class="text-h4 text-primary">
            {{ crescimentoStore.totalMedicoes }}
          </div>
      ...
      <q-card class="col-3">
        <q-card-section class="text-center">
          <div class="text-h4 text-secondary">
            {{ crescimentoStore.animaisComMedicoes }}
          </div>
      ...
      <q-card class="col-3">
        <q-card-section class="text-center">
          <div class="text-h4 text-accent">
            {{ crescimentoStore.mediaGanhoPeso }}
          </div>
      ...
      <q-card class="col-2">
        <q-card-section class="text-center">
          <div class="text-h4 text-positive">
            {{ crescimentoStore.ultimasMedicoes.length }}
          </div>
      ...
      <q-card-section>
        ...
        >
          ...
          <template v-slot:body-cell-ganho_total="props">
            <q-td :props="props">
              <q-chip
                ...
                :color="
                  props.row.ganho_peso_total > 0 ? 'positive' : 'negative'
                "
                text-color="white"
                size="sm"
              >
                {{ props.row.ganho_peso_total > 0 ? '+' : ''
                }}{{ props.row.ganho_peso_total.toFixed(1) }} kg
              </q-chip>
              <span v-else>-</span>
            </q-td>
          </template>
          ...
          <template v-slot:body-cell-ganho_mes="props">
            <q-td :props="props">
              {{
                props.row.ganho_peso_medio_mes
                  ? `${props.row.ganho_peso_medio_mes.toFixed(1)} kg/mês`
                  : '-'
              }}
            </q-td>
          </template>
      ...
      <q-card-section>
        ...
        >
          <template v-slot:body-cell-idade="props">
            <q-td :props="props">{{ props.row.idade_meses }} meses</q-td>
          </template>
...
<script setup>
  ...
  const estatisticasColumns = [
    ...
    },
    {
      name: 'ganho_total',
      label: 'Ganho Total',
      field: 'ganho_peso_total',
      align: 'right',
    },
    {
      name: 'ganho_mes',
      label: 'Ganho/Mês',
      field: 'ganho_peso_medio_mes',
      align: 'right',
    },
