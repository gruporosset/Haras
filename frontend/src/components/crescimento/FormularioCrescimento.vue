            ...
            >
              ...
              >
                <q-card-section class="text-center q-pa-sm">
                  ...
                  <div
                    :class="
                      variacao.peso > 0 ? 'text-positive' : 'text-negative'
                    "
                    class="text-h6"
                  >
                    {{ variacao.peso > 0 ? '+' : ''
                    }}{{ variacao.peso.toFixed(1) }} kg
                  </div>
                </q-card-section>
              </q-card>
            </div>
            ...
            >
              ...
              >
                <q-card-section class="text-center q-pa-sm">
                  ...
                  <div
                    :class="
                      variacao.altura > 0 ? 'text-positive' : 'text-negative'
                    "
                    class="text-h6"
                  >
                    {{ variacao.altura > 0 ? '+' : ''
                    }}{{ variacao.altura.toFixed(1) }} cm
                  </div>
                </q-card-section>
              </q-card>
            </div>
...
<script setup>
  ...
  const variacao = computed(() => {
    ...
    return {
      peso:
        form.value.PESO && ultimaMedicao.value.PESO
          ? form.value.PESO - ultimaMedicao.value.PESO
          : null,
      altura:
        form.value.ALTURA && ultimaMedicao.value.ALTURA
          ? form.value.ALTURA - ultimaMedicao.value.ALTURA
          : null,
    }
  })
