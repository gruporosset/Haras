<template>
  <q-card>
    ...
    <q-card-section v-if="custosProprietarios.length === 0" class="text-center text-grey">
          ...
          <div class="row items-center">
            ...
            <div>
              ...
              <div class="text-caption text-grey">
                {{ props.row.numero_animais }} {{ props.row.numero_animais === 1 ? 'animal' : 'animais' }}
              </div>
            </div>
          </div>
        </q-td>
    ...
    <q-card-section class="bg-grey-1">
      <div class="row q-gutter-md text-center">
        <div class="col">
          <div class="text-h6 text-primary">{{ formatarMoeda(totalGeral) }}</div>
        ...
        <div class="col">
          <div class="text-h6 text-orange">{{ formatarMoeda(mediaPorAnimal) }}</div>
...
export default {
  ...
  setup() {
    ...
    const custosProprietarios = computed(() => dashboardStore.custos_proprietarios)
    ...
    const totalGeral = computed(() => {
      return custosProprietarios.value.reduce((total, item) => total + item.total_geral, 0)
    })
    
    const totalAnimais = computed(() => {
      return custosProprietarios.value.reduce((total, item) => total + item.numero_animais, 0)
    })
    
    const mediaPorAnimal = computed(() => {
      return totalAnimais.value > 0 ? totalGeral.value / totalAnimais.value : 0
    })
