  ...
  <q-dialog v-model="viewDialog" persistent>
    <q-card style="min-width: 600px;">
      ...
      <q-card-section v-if="registroData">
            ...
            <q-list>
              ...
              <q-item>
                <q-item-section>
                  ...
                  <q-item-label>{{ formatDate(registroData.DATA_OCORRENCIA) }}</q-item-label>
                </q-item-section>
              </q-item>
              <q-item>
                <q-item-section>
                  ...
                  <q-chip 
                    ...
                    size="sm"
                  >
                    {{ ferrageamentoStore.getTipoLabel(registroData.TIPO_REGISTRO) }}
                  </q-chip>
                </q-item-section>
              </q-item>
              <q-item v-if="registroData.FERRADOR_RESPONSAVEL">
                <q-item-section>
                  ...
                  <q-item-label>{{ registroData.FERRADOR_RESPONSAVEL }}</q-item-label>
                </q-item-section>
              </q-item>
            </q-list>
          </div>
            ...
            <q-list>
              <q-item v-if="registroData.MEMBRO_TRATADO">
                <q-item-section>
                  ...
                  <q-item-label>{{ ferrageamentoStore.getMembroLabel(registroData.MEMBRO_TRATADO) }}</q-item-label>
                </q-item-section>
              </q-item>
              ...
              <q-item v-if="registroData.CUSTO">
                <q-item-section>
                  ...
                  <q-item-label>R$ {{ registroData.CUSTO.toFixed(2) }}</q-item-label>
                </q-item-section>
              </q-item>
            </q-list>
          </div>
        ...
        <!-- Datas de Controle -->
        <div v-if="registroData.PROXIMA_AVALIACAO || registroData.DATA_PREV_PROX_FERRAGEAMENTO" class="q-mt-md">
          <q-separator />
              ...
              <q-list dense>
                <q-item v-if="registroData.PROXIMA_AVALIACAO">
                  <q-item-section>
                    ...
                    <q-item-label>{{ formatDate(registroData.PROXIMA_AVALIACAO) }}</q-item-label>
                    <q-item-label caption :class="getClasseDiasVencimento(registroData.dias_proxima_avaliacao)">
                      {{ getTextoDiasVencimento(registroData.dias_proxima_avaliacao) }}
                    </q-item-label>
                  </q-item-section>
                </q-item>
              </q-list>
              ...
              <q-list dense>
                <q-item v-if="registroData.DATA_PREV_PROX_FERRAGEAMENTO">
                  <q-item-section>
                    ...
                    <q-item-label>{{ formatDate(registroData.DATA_PREV_PROX_FERRAGEAMENTO) }}</q-item-label>
                  </q-item-section>
                </q-item>
              </q-list>
            </div>
        ...
        <!-- Detalhes Técnicos -->
        <div v-if="temDetalhes" class="q-mt-md">
          <q-separator />
              ...
              <q-list dense>
                <q-item v-if="registroData.PROBLEMAS_IDENTIFICADOS">
                  <q-item-section>
                    ...
                    <q-item-label>{{ registroData.PROBLEMAS_IDENTIFICADOS }}</q-item-label>
                  </q-item-section>
                </q-item>
                <q-item v-if="registroData.TRATAMENTO_REALIZADO">
                  <q-item-section>
                    <q-item-label caption>Tratamento Realizado</q-item-label>
                    <q-item-label>{{ registroData.TRATAMENTO_REALIZADO }}</q-item-label>
                  </q-item-section>
                </q-item>
              </q-list>
            </div>
              ...
              <q-list dense>
                <q-item v-if="registroData.MEDICAMENTOS_UTILIZADOS">
                  <q-item-section>
                    ...
                    <q-item-label>{{ registroData.MEDICAMENTOS_UTILIZADOS }}</q-item-label>
                  </q-item-section>
                </q-item>
                <q-item v-if="registroData.RECOMENDACOES">
                  <q-item-section>
                    <q-item-label caption>Recomendações</q-item-label>
                    <q-item-label>{{ registroData.RECOMENDACOES }}</q-item-label>
                  </q-item-section>
                </q-item>
              </q-list>
            </div>
        ...
        <!-- Observações -->
        <div v-if="registroData.OBSERVACOES" class="q-mt-md">
          <q-separator />
...
const temDetalhes = computed(() => {
  ...
  return registroData.value.PROBLEMAS_IDENTIFICADOS ||
