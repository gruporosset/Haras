  ...
  <div class="analises-solo-container">
    ...
    >
      ...
      <template v-slot:body-cell-saturacao_bases="props">
        <q-td :props="props">
          <div
            v-if="props.row.SATURACAO_BASES"
            class="text-weight-medium"
            :class="getClasseSaturacaoValor(props.row.SATURACAO_BASES)"
          >
      ...
      <template v-slot:body-cell-materia_organica="props">
        <q-td :props="props">
          <div
            v-if="props.row.MATERIA_ORGANICA"
            class="text-weight-medium"
            :class="getClasseMO(props.row.MATERIA_ORGANICA)"
          >
            ...
            >
              <q-card-section class="q-pa-sm">
                ...
                <div class="row q-mt-xs text-body2">
                  <div class="col-6">
                    <div v-if="form.PH_AGUA">
                      <strong>pH:</strong>
                      {{ getStatusPH(form.PH_AGUA) }}
                    </div>
                    <div v-if="form.MATERIA_ORGANICA">
                      <strong>M.O.:</strong>
                      {{ getStatusMO(form.MATERIA_ORGANICA) }}
                    </div>
                  </div>
                  <div class="col-6">
                    <div v-if="form.SATURACAO_BASES">
                      <strong>Sat. Bases:</strong>
                      {{ getStatusSaturacao(form.SATURACAO_BASES) }}
                    </div>
                    <div v-if="form.CTC">
                      <strong>CTC:</strong>
                      {{ getStatusCTC(form.CTC) }}
                    </div>
                  </div>
                </div>
              </q-card-section>
    ...
    <q-dialog v-model="viewDialog">
      <q-card style="min-width: 700px">
        <q-card-section>
          <div class="text-h6">
            Análise de Solo - {{ viewData?.terreno_nome }}
          </div>
        ...
        <q-card-section>
          <div class="row q-gutter-md">
                ...
                <div class="col-md-2 col-6">
                  ...
                  >
                    <q-card-section class="text-center q-pa-sm">
                      <div
                        class="text-h5"
                        :class="getClassePH(viewData?.PH_AGUA)"
                      >
                ...
                <div class="col-md-2 col-6">
                  ...
                  >
                    <q-card-section class="text-center q-pa-sm">
                      <div
                        class="text-h5"
                        :class="getClasseMO(viewData?.MATERIA_ORGANICA)"
                      >
                      ...
                      <div class="text-caption text-grey-6">
                        {{
                          viewData?.MATERIA_ORGANICA
                            ? getStatusMO(viewData.MATERIA_ORGANICA)
                            : ''
                        }}
                      </div>
                    </q-card-section>
                  ...
                  >
                    <q-card-section class="text-center q-pa-sm">
                      <div
                        class="text-h5"
                        :class="
                          getClasseSaturacaoValor(viewData?.SATURACAO_BASES)
                        "
                      >
                      ...
                      <div class="text-caption text-grey-6">
                        {{
                          viewData?.SATURACAO_BASES
                            ? getStatusSaturacao(viewData.SATURACAO_BASES)
                            : ''
                        }}
                      </div>
                    </q-card-section>
            ...
            <div
              ...
              v-if="temMacronutrientes(viewData)"
            >
              ...
              <q-list dense>
                <q-item v-if="viewData?.FOSFORO">
                  ...
                  <q-item-section side>
                    <q-item-label>
                      {{ viewData.FOSFORO.toFixed(1) }} mg/dm³
                    </q-item-label>
                  </q-item-section>
                </q-item>
                <q-item v-if="viewData?.POTASSIO">
                  ...
                  <q-item-section side>
                    <q-item-label>
                      {{ viewData.POTASSIO.toFixed(2) }} cmolc/dm³
                    </q-item-label>
                  </q-item-section>
                </q-item>
                <q-item v-if="viewData?.CALCIO">
                  ...
                  <q-item-section side>
                    <q-item-label>
                      {{ viewData.CALCIO.toFixed(2) }} cmolc/dm³
                    </q-item-label>
                  </q-item-section>
                </q-item>
                <q-item v-if="viewData?.MAGNESIO">
                  ...
                  <q-item-section side>
                    <q-item-label>
                      {{ viewData.MAGNESIO.toFixed(2) }} cmolc/dm³
                    </q-item-label>
                  </q-item-section>
                </q-item>
              </q-list>
            ...
            <div
              class="col-md-6 col-12"
              v-if="temMicronutrientes(viewData)"
            >
              <div class="text-h6 text-primary q-mb-sm">
                Micronutrientes (mg/dm³)
              </div>
...
<script setup>
  ...
  const estatisticas = computed(() => {
    ...
    const saturacoes = analises
      .filter(a => a.SATURACAO_BASES)
      .map(a => a.SATURACAO_BASES)

    const phMedio =
      pHs.length > 0 ? pHs.reduce((sum, ph) => sum + ph, 0) / pHs.length : 0
    const saturacaoMedia =
      saturacoes.length > 0
        ? saturacoes.reduce((sum, sat) => sum + sat, 0) / saturacoes.length
        : 0
    ...
    const ultimaAnalise =
      analises.length > 0
        ? new Date(
            Math.max(...analises.map(a => new Date(a.DATA_COLETA)))
  ...
  const temParametrosPreenchidos = computed(() => {
    return (
      form.value.PH_AGUA ||
      form.value.MATERIA_ORGANICA ||
      form.value.SATURACAO_BASES ||
      form.value.CTC
    )
  })
  ...
  function initializeForm(record) {
    if (record) {
      ...
      if (form.value.DATA_COLETA) {
        form.value.DATA_COLETA = new Date(form.value.DATA_COLETA)
          .toISOString()
          .split('T')[0]
      }
      if (form.value.DATA_RESULTADO) {
        form.value.DATA_RESULTADO = new Date(form.value.DATA_RESULTADO)
          .toISOString()
          .split('T')[0]
      }
    } else {
  ...
  async function deleteAnalise() {
    try {
      ...
      $q.notify({
        type: 'negative',
        message: error.message || 'Erro ao excluir análise',
      })
    }
  }
  ...
  async function submitUpload() {
    ...
    try {
      await manejoStore.uploadLaudoAnalise(
        analiseUpload.value.ID,
      ...
      $q.notify({
        type: 'negative',
        message: error.message || 'Erro ao enviar laudo',
      })
    } finally {
  ...
  function filterTerrenos(val, update) {
    update(() => {
      if (val === '') {
        ...
        terrenoOptions.value = terrenoOptionsDialog.value.filter(t =>
  ...
  function filterTerrenosDialog(val, update) {
    update(() => {
      if (val === '') {
        ...
        const needle = val.toLowerCase()
        terrenoOptionsDialog.value = terrenoOptions.value.filter(t =>
          t.label.toLowerCase().includes(needle)
        )
      }
    })
  ...
  function getStatusResultado(analise) {
    ...
    if (dataResultado > hoje) {
      const diasRestantes = Math.ceil(
        (dataResultado - hoje) / (1000 * 60 * 60 * 24)
      )
  ...
  async function generateAlertas() {
    ...
    analises.forEach(analise => {
      ...
      const temResultadoConcluido =
        analise.DATA_RESULTADO && new Date(analise.DATA_RESULTADO) <= hoje

      if (temResultadoConcluido) {
        if (
          analise.PH_AGUA &&
          (analise.PH_AGUA < 5.5 || analise.PH_AGUA > 7.5)
        ) {
  ...
  function showAllAlertas() {
    $q.notify({
      type: 'info',
      message: 'Modal com todos os alertas em desenvolvimento',
    })
  }
