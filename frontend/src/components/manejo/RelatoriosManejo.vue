<template>
  <div class="relatorios-manejo-container">
      ...
      <q-card-section>
        ...
        <div class="row q-gutter-md">
          <div class="col-md-2 col-6">
            <q-card flat bordered>
              <q-card-section class="text-center q-pa-sm">
                <div class="text-h4 text-primary">{{ estatisticasGerais.totalAplicacoes }}</div>
          ...
          <div class="col-md-2 col-6">
            <q-card flat bordered>
              <q-card-section class="text-center q-pa-sm">
                <div class="text-h4 text-green">{{ estatisticasGerais.terrenosManejados }}</div>
          ...
          <div class="col-md-2 col-6">
            <q-card flat bordered>
              <q-card-section class="text-center q-pa-sm">
                <div class="text-h4 text-orange">{{ manejoStore.formatarMoeda(estatisticasGerais.custoTotal) }}</div>
          ...
          <div class="col-md-2 col-6">
            <q-card flat bordered>
              <q-card-section class="text-center q-pa-sm">
                <div class="text-h4 text-purple">{{ estatisticasGerais.produtosUtilizados }}</div>
          ...
          <div class="col-md-2 col-6">
            <q-card flat bordered>
              <q-card-section class="text-center q-pa-sm">
                <div class="text-h4 text-blue">{{ estatisticasGerais.analisesSolo }}</div>
      ...
      <div class="col-md-6 col-12">
        <q-card>
          <q-card-section>
            ...
            <div class="q-mt-md" style="height: 300px; overflow-y: auto;">
              <q-list v-if="consumoTerreno.length > 0" dense>
                <q-item v-for="item in consumoTerreno.slice(0, 10)" :key="`${item.terreno_id}-${item.produto_nome}`">
                  <q-item-section>
                    <q-item-label>{{ item.terreno_nome }}</q-item-label>
                    <q-item-label caption>{{ item.produto_nome }} - {{ item.tipo_manejo }}</q-item-label>
                  </q-item-section>
                  <q-item-section side>
                    <q-item-label>{{ item.total_aplicado?.toLocaleString('pt-BR') }} {{ item.unidade_medida }}</q-item-label>
                    <q-item-label caption>{{ item.numero_aplicacoes }} aplicações</q-item-label>
                  </q-item-section>
                  <q-item-section side>
                    <q-item-label v-if="item.custo_total">{{ manejoStore.formatarMoeda(item.custo_total) }}</q-item-label>
                  </q-item-section>
                </q-item>
              </q-list>
              ...
              <div v-else class="text-center q-pa-md text-grey-6">
                ...
                <div class="text-caption">Ajuste os filtros ou verifique se há aplicações registradas</div>
              </div>
            </div>
          </q-card-section>
        </q-card>
      ...
      <div class="col-md-6 col-12">
        <q-card>
          <q-card-section>
            ...
            <div class="q-mt-md" style="height: 300px; overflow-y: auto;">
              <q-list v-if="previsaoConsumo.length > 0" dense>
                <q-item v-for="item in previsaoConsumo" :key="item.produto_id">
                  <q-item-section>
                    <q-item-label>{{ item.produto_nome }}</q-item-label>
                    <q-item-label caption>Consumo: {{ item.consumo_mensal_medio?.toFixed(1) }}/mês</q-item-label>
                  </q-item-section>
                  <q-item-section side>
                    <q-item-label>{{ item.estoque_atual?.toLocaleString('pt-BR') }}</q-item-label>
                    <q-item-label caption>{{ item.dias_restantes }} dias restantes</q-item-label>
                  </q-item-section>
                  <q-item-section side>
                    <q-chip
              ...
              <div v-else class="text-center q-pa-md text-grey-6">
                ...
                <div class="text-caption">É necessário histórico de aplicações para gerar previsões</div>
              </div>
            </div>
          </q-card-section>
        </q-card>
      ...
      <div class="col-md-6 col-12">
        <q-card>
          <q-card-section>
            ...
            <div class="q-mt-md" style="height: 300px; overflow-y: auto;">
              <q-list v-if="terrenosLiberacao.length > 0" dense>
                <q-item v-for="item in terrenosLiberacao" :key="`${item.terreno_id}-${item.data_aplicacao}`">
                  <q-item-section>
                    <q-item-label>{{ item.terreno_nome }}</q-item-label>
                    <q-item-label caption>{{ item.produto_nome }} - {{ item.tipo_manejo }}</q-item-label>
                  </q-item-section>
                  <q-item-section side>
                    <q-item-label>{{ formatarData(item.data_liberacao) }}</q-item-label>
                    <q-item-label caption>Aplicado: {{ formatarData(item.data_aplicacao) }}</q-item-label>
                  </q-item-section>
                  <q-item-section side>
                    <q-chip
              ...
              <div v-else class="text-center q-pa-md text-grey-6">
                <q-icon name="check_circle" size="lg" class="q-mb-sm text-positive" />
                <div>Nenhum terreno em carência</div>
                <div class="text-caption">Todos os terrenos estão liberados para uso</div>
              </div>
            </div>
          </q-card-section>
        </q-card>
      ...
      <div class="col-md-6 col-12">
        <q-card>
          <q-card-section>
            ...
            <div class="q-mt-md" style="height: 300px; overflow-y: auto;">
              <q-list v-if="resumoEstoque.length > 0" dense>
                <q-item v-for="item in resumoEstoque.slice(0, 10)" :key="item.produto_id">
                  <q-item-section>
                    <q-item-label>{{ item.produto_nome }}</q-item-label>
                    <q-item-label caption>{{ manejoStore.getTipoLabel(item.tipo_produto) }}</q-item-label>
                  </q-item-section>
                  <q-item-section side>
                    <q-item-label>{{ item.estoque_atual?.toLocaleString('pt-BR') }} {{ item.unidade_medida }}</q-item-label>
                    <q-item-label caption>Mín: {{ item.estoque_minimo?.toLocaleString('pt-BR') }}</q-item-label>
                  </q-item-section>
                  <q-item-section side>
                    <q-item-label v-if="item.valor_entradas">{{ manejoStore.formatarMoeda(item.valor_entradas) }}</q-item-label>
                    <q-linear-progress
              ...
              <div v-else class="text-center q-pa-md text-grey-6">
                ...
                <div class="text-caption">Cadastre produtos para ver o resumo de estoque</div>
              </div>
            </div>
          </q-card-section>
        </q-card>
    ...
    <q-dialog v-model="fullReportDialog" persistent maximized>
      <q-card>
        <q-card-section class="row items-center q-pb-none">
          <div class="text-h6">{{ getFullReportTitle() }}</div>
          <q-space />
          <q-btn icon="close" flat round dense @click="fullReportDialog = false" />
        </q-card-section>
        ...
        <q-card-actions align="right">
          ...
          />
          <q-btn flat label="Fechar" color="grey" @click="fullReportDialog = false" />
        </q-card-actions>
      </q-card>
    </q-dialog>
  </div>
...
const estatisticasGerais = computed(() => {
  ...
  const custoTotal = aplicacoes.reduce((sum, a) => sum + (a.CUSTO_TOTAL || 0), 0)
  
  // Contar alertas ativos
  const alertasEstoque = produtos.filter(p => 
    p.ATIVO === 'S' && (p.ESTOQUE_ATUAL || 0) <= (p.ESTOQUE_MINIMO || 0)
  ).length
  
  const alertasSolo = analises.filter(a => 
  ...
  return {
    ...
    alertasAtivos: alertasEstoque + alertasSolo + terrenosLiberacao.value.length
  }
})
...
const consumoColumns = [
  ...
  { name: 'tipo_manejo', label: 'Tipo', field: 'tipo_manejo', sortable: true, align: 'left' },
  { name: 'total_aplicado', label: 'Total Aplicado', field: 'total_aplicado', sortable: true, align: 'right' },
  { name: 'unidade_medida', label: 'Unidade', field: 'unidade_medida', align: 'center' },
  { name: 'numero_aplicacoes', label: 'Nº Aplicações', field: 'numero_aplicacoes', sortable: true, align: 'center' },
  { name: 'custo_total', label: 'Custo Total', field: 'custo_total', sortable: true, align: 'right' }
]

const previsaoColumns = [
  { name: 'produto_nome', label: 'Produto', field: 'produto_nome', sortable: true, align: 'left' },
  { name: 'consumo_mensal_medio', label: 'Consumo Mensal', field: 'consumo_mensal_medio', sortable: true, align: 'right' },
  { name: 'estoque_atual', label: 'Estoque Atual', field: 'estoque_atual', sortable: true, align: 'right' },
  { name: 'dias_restantes', label: 'Dias Restantes', field: 'dias_restantes', sortable: true, align: 'center' },
  { name: 'data_prevista_fim', label: 'Previsão Fim', field: 'data_prevista_fim', sortable: true, align: 'left' },
  { name: 'recomendacao', label: 'Recomendação', field: 'recomendacao', align: 'center' }
]

const estoqueColumns = [
  { name: 'produto_nome', label: 'Produto', field: 'produto_nome', sortable: true, align: 'left' },
  { name: 'tipo_produto', label: 'Tipo', field: 'tipo_produto', sortable: true, align: 'left' },
  { name: 'estoque_atual', label: 'Estoque Atual', field: 'estoque_atual', sortable: true, align: 'right' },
  { name: 'estoque_minimo', label: 'Estoque Mínimo', field: 'estoque_minimo', sortable: true, align: 'right' },
  { name: 'total_entradas', label: 'Total Entradas', field: 'total_entradas', sortable: true, align: 'right' },
  { name: 'total_saidas', label: 'Total Saídas', field: 'total_saidas', sortable: true, align: 'right' },
  { name: 'valor_entradas', label: 'Valor Entradas', field: 'valor_entradas', sortable: true, align: 'right' },
  { name: 'status_estoque', label: 'Status', field: 'status_estoque', align: 'center' }
]
...
async function loadTerrenosLiberacao() {
  try {
    const data = await manejoStore.getTerrenosLiberacao(diasLiberacao.value.value)
...
function buildParams() {
  ...
  if (filtrosGlobais.value.terrenos?.length > 0) {
    params.terreno_ids = filtrosGlobais.value.terrenos.map(t => t.value).join(',')
  }
...
onMounted(async () => {
  ...
  filtrosGlobais.value.data_inicio = trintaDiasAtras.toISOString().split('T')[0]
