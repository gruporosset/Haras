  ...
  <div class="aplicacoes-terrenos-container">
          ...
          >
            ...
            <div class="row q-gutter-md">
              <q-input
                ...
                :hint="
                  areaTerrenoSelecionado
                    ? `Terreno: ${areaTerrenoSelecionado} HA`
                    : ''
                "
                class="col-3"
              />
            ...
            <div class="row q-gutter-md">
              <q-input
                ...
                :hint="
                  custoCalculado
                    ? `Estimado: ${manejoStore.formatarMoeda(custoCalculado)}`
                    : ''
                "
                class="col-3"
              />
              ...
              <q-input
                ...
                :hint="
                  custoTotalCalculado
                    ? `Calculado: ${manejoStore.formatarMoeda(custoTotalCalculado)}`
                    : ''
                "
                class="col-3"
              />
              <q-input
                ...
                :hint="
                  carenciaRecomendada
                    ? `Recomendado: ${carenciaRecomendada} dias`
                    : ''
                "
                class="col-2"
              />
                ...
                <div class="row q-mt-xs text-body2">
                  <div class="col-6">
                    <div>
                      ...
                      {{ estoqueDisponivel }} {{ unidadeSelecionada }}
                    </div>
                    <div>
                      <strong>Restará:</strong>
                      {{
                        (
                  ...
                  <div class="col-6">
                    <div v-if="dataLiberacao">
                      <strong>Liberação:</strong>
                      {{ dataLiberacao }}
                    </div>
                    <div v-if="custoTotalCalculado">
                      <strong>Custo Total:</strong>
                      {{ manejoStore.formatarMoeda(custoTotalCalculado) }}
                    </div>
                  </div>
                </div>
              </q-card-section>
          ...
          <q-list>
            ...
            <q-item>
              <q-item-section>
                ...
                <q-item-label>
                  <q-chip
                    :color="getCorTipoManejo(viewData?.TIPO_MANEJO)"
                    text-color="white"
                    size="sm"
                  >
              ...
              <q-item-section>
                ...
                <q-item-label>
                  {{ formatarData(viewData?.DATA_APLICACAO) }}
                </q-item-label>
              </q-item-section>
            </q-item>
            ...
            <q-item>
              ...
              <q-item-section>
                ...
                <q-item-label>
                  {{ viewData?.DOSE_HECTARE?.toLocaleString('pt-BR') || '-' }}
                </q-item-label>
              </q-item-section>
            </q-item>
            ...
            <q-item v-if="viewData?.CUSTO_TOTAL">
              <q-item-section>
                ...
                <q-item-label>
                  {{ manejoStore.formatarMoeda(viewData.CUSTO_TOTAL) }}
                </q-item-label>
              </q-item-section>
            </q-item>
            ...
            <q-item v-if="viewData?.PERIODO_CARENCIA">
              ...
              <q-item-section>
                ...
                <q-item-label>
                  {{ formatarData(viewData?.DATA_LIBERACAO) }}
                </q-item-label>
              </q-item-section>
            </q-item>
            ...
            <q-item v-if="viewData?.EQUIPAMENTO_UTILIZADO">
              <q-item-section>
                ...
                <q-item-label>
                  {{ viewData.EQUIPAMENTO_UTILIZADO }}
                </q-item-label>
              </q-item-section>
            </q-item>
    ...
    >
      <q-card>
        ...
        <q-card-section>
          Tem certeza que deseja excluir a aplicação de
          <strong>{{ recordToDelete?.produto_nome }}</strong>
          no terreno
          <strong>{{ recordToDelete?.terreno_nome }}</strong>
          ?
          <div class="text-caption text-warning q-mt-sm">
            <q-icon name="warning" />
            O estoque do produto será restaurado automaticamente.
          </div>
        </q-card-section>
        <q-card-actions align="right">
          <q-btn
...
<script setup>
  ...
  const estatisticas = computed(() => {
    ...
    return {
      ...
      proximasLiberacoes: terrenosCarencia.value.filter(
        t => t.dias_para_liberacao <= 7
      ).length,
    }
  })
  ...
  const columns = [
    ...
    {
      name: 'status_carencia',
      label: 'Status',
      field: 'status_carencia',
      sortable: false,
      align: 'center',
    },
  ...
  function initializeForm(record) {
    if (record) {
      ...
      if (form.value.DATA_APLICACAO) {
        form.value.DATA_APLICACAO = new Date(form.value.DATA_APLICACAO)
          .toISOString()
          .split('T')[0]
      }
    } else {
  ...
  async function submitForm() {
    try {
      ...
      $q.notify({
        type: 'negative',
        message: error.message || 'Erro ao salvar aplicação',
      })
    }
  }
  ...
  async function deleteAplicacao() {
    try {
      ...
      $q.notify({
        type: 'positive',
        message: 'Aplicação excluída e estoque restaurado!',
      })
  ...
  function showAllCarencias() {
    $q.notify({
      type: 'info',
      message: 'Modal com todas as carências em desenvolvimento',
    })
  }

  // Watchers para cálculos automáticos
  watch(
    [() => form.value.CUSTO_PRODUTO, () => form.value.CUSTO_APLICACAO],
    () => {
