  ...
  <div class="calendario-container">
    ...
    >
      <q-card style="width: 400px; max-width: 80vw">
        ...
        <q-card-actions
          align="right"
          v-if="diaSelecionado?.eventos.length > 0"
        >
          <q-btn
            flat
            color="primary"
  ...
  export default {
    ...
    setup(props, { emit }) {
      ...
      const diasMes = computed(() => {
        ...
        const totalDias =
          Math.ceil(
            (ultimoDiaCalendario - primeiroDiaCalendario) /
              (1000 * 60 * 60 * 24)
          ) + 1
      ...
      const carregarEventos = async () => {
        try {
          const response = await saudeStore.calendarioSaude(
            mesAtual.value + 1,
            anoAtual.value
          )
