...
const filterPai = (val, update) => {
  update(() => {
    ...
    paiOptions.value = allAnimals.value.animais.filter(animal => 
      animal.SEXO === 'M' &&
      animal.NOME.toLowerCase().includes(needle)
    ).map(animal => ({
        value: animal.ID,
        label: animal.NOME
      }))
...
const filterMae = (val, update) => {
  update(() => {
    const needle = val.toLowerCase()
    maeOptions.value = allAnimals.value.animais.filter(animal => 
      animal.SEXO === 'F' &&
      animal.NOME.toLowerCase().includes(needle)
    ).map(animal => ({
        value: animal.ID,
...
const formatCpfCnpj = (value) => {
  ...
  if (numbers.length <= 11) {
    // CPF
    form.value.CPF_CNPJ_PROPRIETARIO = numbers.replace(/(\d{3})(\d{3})(\d{3})(\d{2})/, '$1.$2.$3-$4')
  } else {
    // CNPJ
    form.value.CPF_CNPJ_PROPRIETARIO = numbers.replace(/(\d{2})(\d{3})(\d{3})(\d{4})(\d{2})/, '$1.$2.$3/$4-$5')
  }
}
