...
from app.schemas.animal import AnimalCreate, AnimalUpdate, AnimalResponse, AnimalGenealogia, FotoUploadResponse, StatusAnimalEnum, SexoEnum
...
async def create_animal(
    ...
    if animal.NUMERO_REGISTRO:
        existing = db.query(Animal).filter(Animal.NUMERO_REGISTRO == animal.NUMERO_REGISTRO).first()
        if existing:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Número de registro já existe")
    
    # Verificar se chip já existe
    if animal.CHIP_IDENTIFICACAO:
        existing = db.query(Animal).filter(Animal.CHIP_IDENTIFICACAO == animal.CHIP_IDENTIFICACAO).first()
        if existing:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Chip de identificação já existe")
...
async def list_animais(
    ...
    numero_registro: Optional[str] = Query(None, description="Filtrar por número de registro"),
    ...
    # Aplicar ordenação
    valid_sort_fields = ["ID", "NOME", "DATA_NASCIMENTO", "SEXO", "STATUS_ANIMAL", "NUMERO_REGISTRO"]
...
async def get_animal(id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
async def update_animal(
    ...
    if animal.NUMERO_REGISTRO and animal.NUMERO_REGISTRO != db_animal.NUMERO_REGISTRO:
        existing = db.query(Animal).filter(Animal.NUMERO_REGISTRO == animal.NUMERO_REGISTRO, Animal.ID != id).first()
        if existing:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Número de registro já existe")
    
    if animal.CHIP_IDENTIFICACAO and animal.CHIP_IDENTIFICACAO != db_animal.CHIP_IDENTIFICACAO:
        existing = db.query(Animal).filter(Animal.CHIP_IDENTIFICACAO == animal.CHIP_IDENTIFICACAO, Animal.ID != id).first()
        if existing:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Chip de identificação já existe")
...
async def delete_animal(id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    db_animal = db.query(Animal).filter(Animal.ID == id).first()
    if not db_animal:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Animal não encontrado")
...
async def get_genealogia(id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
async def upload_foto(
    ...
    if not foto.filename:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="O arquivo enviado não possui um nome válido")
...
async def get_parent_options(
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
    sexo: Optional[SexoEnum] = Query(None, description="Filtrar por sexo para parentesco")
):
    ...
    return [
        {"value": animal.ID, "label": f"{animal.NOME} ({animal.NUMERO_REGISTRO or 'S/R'})"}
