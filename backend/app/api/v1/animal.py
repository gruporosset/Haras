...
async def create_animal(
    ...
    if animal.NUMERO_REGISTRO:
        existing = db.query(Animal).filter(
            Animal.NUMERO_REGISTRO == animal.NUMERO_REGISTRO).first()
        if existing:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Número de registro já existe")

    # Verificar se chip já existe
    if animal.CHIP_IDENTIFICACAO:
        existing = db.query(Animal).filter(
            Animal.CHIP_IDENTIFICACAO == animal.CHIP_IDENTIFICACAO).first()
        if existing:
...
async def list_animais(
    ...
    nome: Optional[str] = Query(
        None, description="Filtrar por nome do animal"),
    sexo: Optional[SexoEnum] = Query(None, description="Filtrar por sexo"),
    status: Optional[StatusAnimalEnum] = Query(
    ...
    if numero_registro:
        query = query.filter(
            Animal.NUMERO_REGISTRO.ilike(f"%{numero_registro}%"))
    if chip:
    ...
    # Aplicar ordenação
    valid_sort_fields = ["ID", "NOME", "DATA_NASCIMENTO",
                         "SEXO", "STATUS_ANIMAL", "NUMERO_REGISTRO"]
...
async def update_animal(
    ...
    if animal.NUMERO_REGISTRO and animal.NUMERO_REGISTRO != db_animal.NUMERO_REGISTRO:
        existing = db.query(Animal).filter(
            Animal.NUMERO_REGISTRO == animal.NUMERO_REGISTRO, Animal.ID != id).first()
        if existing:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Número de registro já existe")
    ...
    if animal.CHIP_IDENTIFICACAO and animal.CHIP_IDENTIFICACAO != db_animal.CHIP_IDENTIFICACAO:
        existing = db.query(Animal).filter(
            Animal.CHIP_IDENTIFICACAO == animal.CHIP_IDENTIFICACAO, Animal.ID != id).first()
        if existing:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,
                                detail="Chip de identificação já existe")
...
async def get_parent_options(
    ...
    query = db.query(Animal).filter(
        Animal.STATUS_ANIMAL == StatusAnimalEnum.ATIVO)
