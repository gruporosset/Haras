...
async def create_movimentacao(
    ...
    if movimentacao.ID_TERRENO_ORIGEM:
        terreno_origem = db.query(Terreno).filter(
            Terreno.ID == movimentacao.ID_TERRENO_ORIGEM).first()
        if not terreno_origem:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                                detail="Terreno de origem não encontrado")

    if movimentacao.ID_TERRENO_DESTINO:
        terreno_destino = db.query(Terreno).filter(
            Terreno.ID == movimentacao.ID_TERRENO_DESTINO).first()
...
async def list_movimentacoes(
    ...
    if tipo_movimentacao:
        query = query.filter(
            MovimentacaoAnimais.TIPO_MOVIMENTACAO == tipo_movimentacao)
    if terreno_id:
        query = query.filter(
    ...
    movimentacoes = query.order_by(
        desc(MovimentacaoAnimais.DATA_MOVIMENTACAO)).offset(offset).limit(limit).all()
...
async def get_movimentacao(id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    movimentacao = db.query(MovimentacaoAnimais).filter(
        MovimentacaoAnimais.ID == id).first()
    if not movimentacao:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
...
async def update_movimentacao(
    ...
    db_movimentacao = db.query(MovimentacaoAnimais).filter(
        MovimentacaoAnimais.ID == id).first()
    ...
    if movimentacoes:
        ...
        if ultima.ID_TERRENO_DESTINO:
            terreno = db.query(Terreno).filter(
                Terreno.ID == ultima.ID_TERRENO_DESTINO).first()
...
async def get_localizacoes_atuais(
    ...
    for mov, animal_nome in ultimas_movimentacoes:
        ...
        if mov.ID_TERRENO_DESTINO:
            terreno = db.query(Terreno).filter(
                Terreno.ID == mov.ID_TERRENO_DESTINO).first()
